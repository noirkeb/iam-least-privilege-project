...
AWS IAM 평가 로직상 명시적 Deny는 다른 Allow보다 항상 우선 적용되므로, 향후 다른 정책이 실수로 prod 리소스에 대한 권한을 허용하더라도 이 Deny 규칙이 안전장치 역할을 합니다.
...
### dev 리소스 삭제 시도 (Allowed)

![dev allowed](screenshots/simulator-dev-allowed.png)

### prod 리소스 삭제 시도 (Denied)

![prod denied](screenshots/simulator-prod-denied.png)
